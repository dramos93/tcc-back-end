...
from game.data.interfaces.authentication_repository_interface import AuthenticationRepositoryInterface
...
class AuthenticationRepository(AuthenticationRepositoryInterface):
    ...
    def create(cls, user_id: int) -> None:
        with DBConnectionHandler() as db:    
            try:
                ...
                query = update(AuthenticationEntity).filter(AuthenticationEntity.user_id == user_id).values(active=False)
    ...
    def get_token(cls, user_id: int) -> AuthenticationModel:
        with DBConnectionHandler() as db:
            try:
                query = select(AuthenticationEntity).filter((AuthenticationEntity.active) & (AuthenticationEntity.user_id == user_id))
    ...
    def logout(cls, user_id: int, token: UUID) -> None:
        with DBConnectionHandler() as db:
            try:
                query = update(AuthenticationEntity).filter((AuthenticationEntity.user_id == user_id) & (AuthenticationEntity.token == token)).values(active=False)
                db.session.execute(query)
                db.session.commit()
            except Exception as exception:
                breakpoint()
